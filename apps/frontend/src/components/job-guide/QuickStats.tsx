...
export function QuickStats({ phases, overallProgress }: QuickStatsProps) {
  ...
  const completedPhases = phases.filter(
    (p) => p.items.length > 0 && p.items.every((i) => i.isCompleted),
  ).length;

  const currentPhase = phases.find(
    (p) => p.items.length > 0 && p.items.some((i) => i.isCompleted) && !p.items.every((i) => i.isCompleted),
  );
  ...
  return (
    <div className="mb-8 space-y-4">
      ...
      {currentPhase &&
        currentPhase.items.filter((i) => i.isCompleted).length ===
